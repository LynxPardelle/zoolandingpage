...
- 2026-06-04 16:10 CT, updated 17:12 CT: Loading-curtain readiness for Angora drafts must account for rendered combo CSS, not just "some CSS rules exist." Before explicit `cssCreate` class updates, sync draft combos from `ConfigStoreService`, split authored class strings into individual tokens, and keep the curtain active until the current DOM has rendered combo classes such as `sectionTitle` and those combos have expanded `__COM_*` rules. Re-sample the DOM inside the readiness loop, because the first hide request can run before dynamic roots are in the DOM. Use a timer fallback around `requestAnimationFrame` in readiness code so automated/background browsers cannot leave the curtain permanently stuck.
