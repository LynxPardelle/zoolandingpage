...
**Verified:** 2026-07-21 (Central Time) for the Phase 6 local service entries; previously listed GitHub metadata was not reclassified by this documentation closeout.
...
| `zoolanding-data-spaces` (local only; no GitHub remote) | Generic draft-scoped collection schemas, records, immutable revisions, published projections, and Phase 8 deployment-readiness controls | n/a | Local `AGENTS.md`, then README; CI/infra are ready locally, the internal caller is disabled, and AWS deployment remains blocked |
| `zoolanding-commerce` (local only; no GitHub remote) | Provider-neutral catalog, inventory, Checkout, subscription, migration requests/approval, Integrations gateway, exact notification event/outbox, reconciliation, isolated manual fiscal workflows, and Phase 8 deployment-readiness controls | n/a | Local `AGENTS.md`, then README; CI/infra are ready locally, while stream-policy test evidence and AWS deployment remain blocked |
| `zoolanding-integrations` (local only; no GitHub remote) | Generic connection registry, Stripe Connect onboarding/adapter, SMTP registration/activation/resolution, provider mappings, hosted Checkout/portal commands, webhook normalization, event outbox, bounded migration, and Phase 8 deployment-readiness controls | n/a | Local `AGENTS.md`, then README; CI/infra are ready locally, while two-pass caller bootstrap, provider activation, and AWS deployment remain blocked |
| `zoolanding-notifications` (local only; no GitHub remote) | Draft-scoped outbound notification policy replay, recipient/SMTP resolution, delivery ledger, rate/circuit state, SMTP2GO adapter, queue/DLQ, alarms, reconciliation, and Phase 8 deployment-readiness controls | n/a | Local `AGENTS.md`, then README; CI/infra are ready locally, while secrets, SMTP evidence, and AWS deployment remain blocked |
...
| [`zoolandingpage-aws-infra`](https://github.com/LynxPardelle/zoolandingpage-aws-infra) | Frontend SSR release infrastructure, OIDC publication roles, Lambda, CloudFront, optional DNS aliases, and exact Phase 8 integration-service origins/routes | `dev` | Repo README, runbooks, and local agent instructions; Phase 8 changes remain in a local worktree and are not deployed |
