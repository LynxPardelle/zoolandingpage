...
| `zoolanding-commerce` (local only; no GitHub remote) | Commercial policy resolution plus an undeployed three-table foundation for catalog, operations, and isolated fiscal requests; domain behavior remains planned | n/a | Local `AGENTS.md`, then README; TASK-025/TASK-026 are local-only and AWS deployment remains blocked |
