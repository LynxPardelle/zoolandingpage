...
| `zoolanding-commerce` (local only; no GitHub remote) | Commercial policy resolution, an undeployed three-table foundation, provider-neutral domain invariants, and immutable catalog/offer/discount versions | n/a | Local `AGENTS.md`, then README; TASK-025 through TASK-028 are local-only and AWS deployment remains blocked |
