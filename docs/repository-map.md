...
| `zoolanding-commerce` (local only; no GitHub remote) | Provider-neutral catalog, inventory, Checkout, subscription, migration requests/approval, Integrations gateway, exact notification event/outbox, reconciliation, and isolated manual fiscal workflows | n/a | Local `AGENTS.md`, then README; Phase 6 notification projection is complete locally and AWS deployment remains blocked |
| `zoolanding-integrations` (local only; no GitHub remote) | Generic connection registry, Stripe Connect onboarding/adapter, SMTP registration/activation/resolution, provider mappings, hosted Checkout/portal commands, webhook normalization, event outbox, and bounded bulk subscription migration | n/a | Local `AGENTS.md`, then README; Phase 6 SMTP connection control is complete locally and AWS/provider activation remains blocked |
| `zoolanding-notifications` (local only; no GitHub remote) | Draft-scoped outbound notification policy replay, recipient/SMTP secret resolution, delivery ledger, rate/circuit state, SMTP2GO adapter, queue/DLQ, alarms, and manual reconciliation boundary | n/a | Local `AGENTS.md`, then README; Phase 6 is complete locally and AWS/SMTP activation remains blocked |
