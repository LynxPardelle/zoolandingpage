...
| Commerce service | TASK-025 through TASK-029 are implemented and verified in the local `Z:\GitHub\zoolanding-commerce` repository: PAT-007, an undeployed three-table storage foundation, provider-neutral domain invariants, immutable catalog/offer/discount versions, and conditional inventory transactions; handlers, routes, event-source mappings, queues, roles, indexes, remote CI, and deployed AWS resources do not exist yet |
