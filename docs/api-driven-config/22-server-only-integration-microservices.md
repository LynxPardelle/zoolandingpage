...
Date: 2026-07-22 (Central Time)
Scope: Draft-scoped server-only descriptors and the approved service boundaries that consume them.
Status: Phase 1 contract/authoring boundary is active; Data Spaces, Commerce, Integrations, Notifications, the generic Zoolandingpage clients, and Phase 8 deployment-readiness infrastructure are implemented locally only; no new service stack, route, secret, or live provider path is deployed by this document.
Source Of Truth:
...
Confidence: High for the Phase 1 descriptor/readiness/authoring contract and the verified local Data Spaces, Commerce, Integrations, Notifications, browser-client, exact-routing, CI/CD, IAM, observability, and smoke implementations through Phase 8; unverified for remote deployment and provider-backed behavior.
Last Reviewed: 2026-07-22 (Central Time)
...
| Data Spaces service | Implemented and verified locally in `Z:\GitHub\zoolanding-data-spaces`, including protected release workflows, exact SSM/IAM dependencies, alarms, throttling, and redacted readiness evidence; its internal snapshot caller remains deliberately disabled and no remote or AWS resource exists yet |
| Commerce service | TASK-025 through TASK-034, Phase 5 migration, Phase 6 notification projection, and Phase 8 release infrastructure are implemented and verified locally in `Z:\GitHub\zoolanding-commerce`: policy resolution, retained storage, eight literal browser routes, authorization, catalog/inventory/Checkout, event inbox/outbox, subscription/reconciliation, manual fiscal intake, exact SSM/IAM, alarms, and readiness evidence; no remote or AWS deployment exists yet |
| Integrations service | TASK-035 through TASK-053 plus SMTP control and Phase 8 release infrastructure are implemented and verified locally in `Z:\GitHub\zoolanding-integrations`: connection/Stripe boundaries, provider mappings, webhook/outbox processing, subscription/migration commands, exact callers, bounded webhook admission/concurrency, alarms, and readiness evidence; no remote, AWS deployment, webhook, or provider activation exists yet |
| Notifications service | TASK-054 through TASK-058 plus Phase 8 release infrastructure are implemented and verified locally in `Z:\GitHub\zoolanding-notifications`: exact-version policy replay, closed events, recipient/SMTP resolution, bounded delivery state, queue/DLQ, exact cross-service identity checks, alarms, operator reconciliation, and readiness evidence; no remote, AWS deployment, secret, or SMTP delivery exists yet |
| Zoolandingpage generic clients | TASK-059 through TASK-065 are implemented and verified locally: exact `data-space`, `commerce`, and `integrations` bindings, same-origin clients, public/protected/SSR separation, CSRF and mutation idempotency, transient OAuth/Stripe handoffs, safe localized errors, generic dispatch, schemas, and synthetic authoring examples; no draft-specific page, provider call, route deployment, or central administration UI was added |
| Stripe Connect, Checkout, Billing, webhook handling, and bulk migration | Stripe adapter and migration behavior are implemented and contract-tested locally; live setup and provider-backed evidence remain gated |
...
SMTP2GO is the first Notifications transport, not the notification contract. Draft descriptors continue to request the generic `email.smtp` provider and `accepted_by_smtp` result; the owning services select the code-owned adapter `smtp2go-smtp-v1`, the fixed endpoint `mail.smtp2go.com` on port `465` with implicit TLS, the account binding, and the credential. Test uses the standalone SMTP2GO account designated for `zoolandingpage.com.mx`, once provisioned and verified, with a unique SMTP user/credential, connection, and server-enforced sender/rate namespace per test draft; production requires the draft's canonical domain and never inherits the shared test sender. Public plan documentation lists unlimited SMTP users; if the live account contradicts that entitlement, activation fails closed and the plan is repriced rather than sharing a credential. Production uses one standalone SMTP2GO account and credential set per `draftId + canonical sending domain`; production account or credential reuse across drafts or sending domains is forbidden. A second production sending domain requires a separately approved connection rather than implicit reuse.
...
```json
{
  ...
  "bindings": [{
    ...
    "status": "active",
    "mode": "test",
    "capabilities": ["connect-onboarding", "checkout", "subscriptions", "customer-portal"],
    "stripe": {
      ...
      "customerPortalReturnPath": "/admin/suscripcion",
      "onboardingRoutes": {
...
This synthetic example is valid for local/test authoring only when the referenced server-only Auth Profile satisfies the group policy; it is not a production approval. `connect-onboarding` requires `integration:manage` and both onboarding route values are bounded same-origin paths, never URLs or origins. `customerPortalReturnPath` is likewise a bounded same-origin path, is required if and only if the binding declares `customer-portal`, and is forbidden otherwise. The one Stripe-only onboarding endpoint remains `POST /features/integrations/stripe/onboarding` with the closed operations `start|return|deauthorize`. Every operation requires the same scoped active Auth Profile, fresh account state, CSRF for mutation, and `integration:manage`. `start` accepts only `bindingId`. OAuth `return` accepts `bindingId`, one-time `state`, and exactly one of `code` or sanitized `error`; the error is a closed non-sensitive code, never provider text. The service validates and consumes the one-time `state` before acting on either outcome. It exchanges `code` only server-side. OAuth access and refresh tokens exist only in that server exchange and are immediately discarded; they are never accepted from or returned to the browser, stored, or logged. Controller return accepts only `bindingId` and one-time `state`, with neither OAuth outcome field. `deauthorize` accepts only `bindingId`, disables the server-resolved connection and routing claim, and never accepts an account ID or token. The browser never supplies account IDs, return URLs, controller properties, ownership, client credentials, or proof of readiness.

Integrations derives immutable `accountOwnership` from the selected strategy: `external-oauth` for `oauth-standard-v1` and `platform-controller` for `controller-account-link-v1`. It stores ownership and only a safe account-reference mapping server-side, re-fetches canonical status after return, and exposes only sanitized readiness and requirement summaries. Generic `requestReconnect` is a same-account retry only: it may reuse the unchanged server-owned account reference, but changing the account or strategy requires `deauthorize` to remove the old account/routing claim before another onboarding flow.

That generic browser lifecycle does not apply to `email.smtp` in the Phase 6 MVP. Browser `disable` and `requestReconnect` operations for SMTP fail closed before Registry mutation because credential rotation, claim transfer, and reactivation require an explicit atomic operator workflow that is not yet implemented. Urgent SMTP revocation uses the active published policy plus the current `zoolanding:enabled=false` secret lifecycle tag. Only the dedicated AWS_IAM activation path may activate a pending SMTP registration, and only its separate operator caller allowlist may invoke it; the Notifications resolver identity has no activation authority.

A draft-supplied `taxApprovalId` is only an opaque lookup key and never proves approval. Before every production subscription mutation, Integrations must verify a server-owned Registry approval record scoped exactly to `environment`, `tenantId`, `draftId`, `domain`, `connectionId`, `accountHash`, `mode`, and the mutation `revision`. A missing, stale, mismatched, or absent record fails closed before provider access. Tests may use an injected verifier, but production wiring must use the Registry record and cannot accept a draft, request, environment variable, or dependency-injection flag as proof. `platformFeeMode` remains `disabled`.

Customer Portal return URLs are never accepted from a browser. Integrations composes the absolute return URL from the exact published canonical domain behind the approved front door plus the reviewed `customerPortalReturnPath`; request `Host`, forwarded-origin headers, provider values, arbitrary origins, query strings, fragments, and provider IDs cannot change it.
...
One policy contains exactly one recipient-set version and that version contains exactly one recipient in the MVP. The actual address is provisioned separately as a secret and is never printed or copied into draft files. Phase 1 supports only `payment-succeeded` -> `payment-succeeded-v1` and `payment-failed` -> `payment-failed-v1`; type/template sets must match exactly. At most 20 unique active SMTP plus recipient secret references may be checked per package. A draft-supplied `transportApprovalId` never opens production; the server-controlled transport live gate remains blocking.
...
The four migration paths are implemented local Phase 5 boundaries. Preview creates a provider-read-only asynchronous dry run; execute requires its exact unexpired revision/hash and server-owned authorization; control is bound to the expected job revision; and protected status reads are paginated. None is deployed or provider-activated by this document.
...
Commerce and Integrations share the same code-owned runtime mutation bounds. `amountMinor` is an integer no greater than 99,999,999; revisions and integer counters are no greater than 9,999,999,999; presentation `displayName` is at most 160 characters and rejects URLs, bare domains, control/format (`Cc`/`Cf`) characters, and bidi controls; a repeating discount lasts at most 36 months; and a redemption limit is at most 1,000,000. These are runtime mutations, not draft descriptor fields or a draft-authored catalog seed. Commerce validates them before Commerce persistence, and Integrations independently revalidates the provider command before any provider mutation.

The individual Phase 4 `next-renewal` command remains deliberately narrow to an unambiguous one-item subscription without an existing Schedule. The Phase 5 bulk engine owns general supported reconstruction: its dry run snapshots the exact target item, quantity, current/future schedule, invoice/payment state, discounts, tax settings, and provider revision; execution rebuilds every supported item and phase, changes only the selected target Price, preserves explicit quantities and allowlisted settings, and refuses more than ten phases or any state it cannot reproduce losslessly. `immediate-prorated` reuses the exact preview timestamp and positive amount, applies only to eligible automatic card/Link collection, and routes unpaid, ambiguous, conflicting, or unsafe recovery state to `needs_review`. Work selection, canary release, retries, cancellation, and rollback remain bounded; only an owned next-renewal Schedule may be released or restored, never canceled as a substitute for rollback.

### Stripe webhook ingress

`/webhooks/stripe/connect` is a direct Integrations API Gateway path. It must not traverse draft domains or the frontend CloudFront distribution. The handler verifies the signature over unmodified bytes and configured timestamp tolerance before using the signed account ID. One immutable, non-authorizing sentinel keyed by `environment + mode + hash(accountId)` routes to the exact scoped connection; the handler then revalidates environment, tenant, draft, connection, account, and `livemode`. A second immutable global replay sentinel keyed by `environment + provider + eventId` binds the scoped reference, account hash, mode, and payload hash so the SEC-006B matrix is enforceable across drafts. Supported events atomically store only allowlisted receipt metadata, payload hash, and ingress outbox. The full Stripe payload is never persisted. Phase 5 extends the original 12-event boundary with Stripe's two pending-update terminal events, producing one exact 14-event allowlist. The list includes Stripe's Connect deauthorization event following the official [Stripe Connect webhook guidance](https://docs.stripe.com/connect/webhooks). A valid signed event outside the following exact allowlist returns controlled `2xx` with no receipt/outbox work, provider re-fetch, provider-state mutation, or Commerce event:
...
| `customer.subscription.created`, `customer.subscription.updated`, `customer.subscription.deleted`, `customer.subscription.pending_update_applied`, `customer.subscription.pending_update_expired` | Event, then Subscription and latest Invoice when present | `commerce.subscription.updated.v1`; the two pending-update terminal events also reconcile the exact migration item without trusting the webhook payload |
...
2. Each service normally resolves its own descriptor from the Config Registry pointer for `environment + draft`, then loads the exact immutable object under that published version. Cache keys include `environment + draft + versionId`; missing or mismatched policy fails closed. Notifications is the deliberate queued-event exception: it revalidates fresh Registry metadata and scope, then reads the exact historical manifest and exact descriptor named by the event's `publishedVersionId`, verifies the manifest path/kind/hash, and never consults or falls back to the current pointer for that event.
...
7. Commerce publishes `notification.requested.v1` only after a confirmed state change. Notifications refreshes Registry scope and resolves the policy only from that event's exact `publishedVersionId`; a warm runtime may reuse only the immutable manifest/hash-verified selection keyed by the complete event tuple. It rechecks current secret lifecycle tags, resolves the connection through AWS IAM, enforces the bound draft/domain sender policy, and attempts SMTP over TLS through the code-owned SMTP2GO endpoint.
8. A payment or notification never publishes, unpublishes, suspends, or deletes a draft.

## Event Contract

Detailed JSON Schemas remain owned by the emitting and consuming services. Integrations reuses the four Commerce consumer shapes unchanged and adds only the approved Phase 5 migration result family; the envelope constraints are fixed:
...
The first confirmed consumer event is `notification.requested.v1`. Its closed data object contains `notificationPolicyId`, `notificationType`, exact `publishedVersionId`, code-owned `templateId`, `recipientSetId`, immutable `recipientSetVersion`, one opaque `recipientMemberId`, `source`, `dedupeKey`, and only the typed variables `orderId`, `amountMinor`, and `currency`; the envelope carries environment/draft/tenant and event identity. The only type/template pairs are `payment-succeeded` -> `payment-succeeded-v1` and `payment-failed` -> `payment-failed-v1`. Commerce derives this authority from its exact immutable policy package, includes the complete target in its Checkout request hash, omits a notification whose type is not configured, and hashes the complete canonical event data except `dedupeKey` into the event/dedupe identity. It excludes addresses, message bodies, fiscal fields, credentials, provider payloads, and provider identifiers.

Integrations emits exactly the four implemented Commerce consumer contracts: `commerce.payment.succeeded.v1`, `commerce.payment.terminal_unpaid.v1`, `commerce.refund.confirmed.v1`, and `commerce.subscription.updated.v1`. It does not create unconsumed generic `payment.*`, `checkout.*`, `refund.*`, `subscription.*`, or `account.*` families. Phase 5 also implements the exact versioned migration results `migration.preview_ready.v1`, `migration.progressed.v1`, `migration.item_needs_review.v1`, and `migration.completed.v1` through the existing outbox/topic. Those events carry scoped opaque identifiers and redacted progress/result data; protected per-item detail remains behind the authorized paginated status route.
...
- SMTP cannot provide exactly-once delivery. A crash after SMTP accepts but before ledger commit becomes ambiguous; the message can be duplicated only if an operator explicitly authorizes another attempt after investigation. Status stops at `accepted_by_smtp`, never guaranteed inbox delivery.
- Notifications writes `prepared` and `sending` ledger states before the SMTP attempt. A confirmed SMTP `4xx` may retry and a confirmed `5xx` fails permanently; a timeout, connection loss, stale `sending` lease, or crash with no explicit rejection becomes `uncertain` with no blind resend. An operator must manually investigate the matching SMTP2GO dashboard activity and record the decision before authorizing another attempt.
...
## Phase 8 Local Infrastructure Readiness

Phase 8 prepares deployment; it does not perform it. The four service repositories now contain all-branch CI, protected `dev -> test` and `test -> main` deployment workflows, exact candidate artifacts, OIDC only in the dependent deploy job, environment-scoped SSM identifiers, least-privilege runtime roles, alarms, and redacted service smokes. `dev` still has no AWS stack, profile, deployment Environment, or deploy workflow. Config Authoring revalidates the same server descriptors immediately before any S3 write, Auth Admin publishes only non-secret table identifiers, and the frontend infrastructure worktree routes the 14 literal browser paths through HTTPS origins with caching disabled. The direct Stripe webhook and all internal service routes remain outside frontend CloudFront.

Bootstrap is deliberately two-pass. A first Integrations stack creation may use an empty internal-caller list and therefore denies every internal route. Commerce and Notifications can then publish their exact caller roles; a later Integrations update must read and include the four Commerce roles plus the Notifications worker role. An existing Integrations stack cannot be updated with an empty list. Data Spaces may remain deployed with its internal snapshot caller empty; that route stays disabled until a dedicated Commerce caller and exact invoke grant are implemented and approved.

Detailed API Gateway metrics are enabled only for the bounded public methods needed to observe the native throttle: one Integrations webhook method, one Data Spaces public-read method, and three Commerce public methods. Their method-scoped `4XXError` alarms combine 400, 403, and 429 responses and therefore do not prove throttling. Before deployment, the economic gate must compare their CloudWatch detailed-metric cost against a cheaper, equally redacted signal; controlled load evidence is still required. No WAF was added.

### Combined readiness smoke runbook

The hub command is an aggregator, not an end-to-end executor. After an explicitly authorized deployment, an operator must run each owning service smoke in the same authenticated shell and capture its single JSON stdout line. Each service result includes only a closed readiness classification/category, validated `test|production` environment (or `null` before input validation), observation epoch, and the attempt count or optional HTTP status only when that owning smoke emits them. It contains no credential, domain, record, customer, fiscal, provider, or secret value.

For `test`, after setting each service's documented smoke inputs and `AWS_REGION`, run:

```powershell
$env:ZLP_INTEGRATION_PLATFORM_SMOKE_ENVIRONMENT = 'test'
$env:ZLP_DATA_SPACES_SMOKE_RESULT_JSON = (python Z:\GitHub\zoolanding-data-spaces\tools\data_spaces_readiness_smoke.py | Out-String).Trim()
$env:ZLP_COMMERCE_SMOKE_RESULT_JSON = (python Z:\GitHub\zoolanding-commerce\tools\commerce_readiness_smoke.py | Out-String).Trim()
$env:ZLP_INTEGRATIONS_SMOKE_RESULT_JSON = (python Z:\GitHub\zoolanding-integrations\tools\integration_platform_readiness_smoke.py | Out-String).Trim()
$env:ZLP_NOTIFICATIONS_SMOKE_RESULT_JSON = (python Z:\GitHub\zoolanding-notifications\tools\notification_readiness_smoke.py --environment test --region $env:AWS_REGION | Out-String).Trim()
npm run integration-platform:smoke
```

A service command intentionally exits nonzero for a non-ready classification while still emitting its one redacted JSON line; capture that line without substituting a hand-written status. The hub accepts only bounded JSON, requires all four results to match the declared environment, rejects inconsistent `ok` claims, rejects observations older than 15 minutes or more than 60 seconds in the future, applies deterministic closed failure precedence, and never reflects raw input. Re-run all four smokes rather than reusing stale output. Clear the five `ZLP_*_SMOKE_*` environment variables after the check. Do not save them with credentials or other runtime environment values in logs, repository files, shell profiles, or CI artifacts.
...
Service unit/contract tests cover generic Integrations `adminAccess`, separation from provider capabilities, cross-draft and wrong-environment denial, Accounts v2 blocked without Mexico Sandbox proof, Accounts v1/OAuth fallback, immutable hashed account routing, the exact 14-event Stripe allowlist and global signature/replay handling, absence of full provider payloads, exact four-event Commerce emission, internal-only deauthorization handling, safe unsupported-event acknowledgement, provider timeout/idempotency, `pause_collection` without a false paused-subscription status, and inventory contention/reservation reconciliation. Phase 5 additionally covers non-mutating preview and exact approval, interruption/resume/canceling, durable canary state, bounded work selection, duplicate delivery, stale snapshots, multi-item schedule preservation, immediate-proration eligibility, pending update/customer action, partial failure, backoff, rollback/reverse migration, event replay, and cross-scope denial. Phase 6 covers exact historical policy replay, complete event/recipient tuples, SMTP registration and operator-only activation, production account/credential claim isolation, test sender-domain enforcement, fresh secret revocation, duplicate-worker ownership, stale/ambiguous delivery without blind resend, fixed implicit TLS endpoint/port, SMTP error classes and circuits, bounded rates, retained ledger state, and queue/DLQ/alarm contracts. Phase 7 covers exact frontend bindings and route dispatch, public/protected and SSR separation, operation-specific input projection, transient OAuth/provider handoffs, safe errors, CSRF, and ambiguity-safe mutation idempotency. Phase 8 covers promotion ancestry, exact immutable artifacts, no-dev enforcement, literal routing, SSM ownership, same-account/region ARN checks, least-privilege IAM, queue policies, alarms, bounded webhook/public admission, and fresh same-environment redacted smoke evidence. Later phases must still add authorized remote deployment, cross-service failure injection, provider-backed acceptance evidence, load tests, and desktop/mobile browser QA for every affected pilot route.
...
- Phases 4 through 8 are local-only. Phase 8 prepared, but did not create, live connected accounts, webhook endpoints, secrets, stacks, routes, queues, tables, or other AWS/provider resources. Pilot configuration remains Phase 9; the first authorized test deployment and evidence rollout remain Phase 10 TASK-087. Both require explicit authorization. The retained Commerce `MigrationRequest` history also requires an approved production retention/deletion policy, access policy, and cleanup runbook before activation.
...
5. Remote repositories and protected environments for the four new services, exact OIDC/CloudFormation identities, upstream SSM values, explicit deployment authorization, test stack evidence, live alarms, and fresh cross-service smoke evidence. The corresponding code is implemented and audited locally only.
