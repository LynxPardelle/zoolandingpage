...
Status: Phase 1 contract/authoring boundary is active; Data Spaces, Commerce, Integrations, Notifications, and the generic Zoolandingpage clients through Phase 7 are implemented locally only; no new service stack or live provider path is deployed by this document.
Source Of Truth:
...
Confidence: High for the Phase 1 descriptor/readiness/authoring contract and the verified local Data Spaces, Commerce, Integrations, Notifications, and browser-client implementations through Phase 7; planned for live service infrastructure and provider-backed behavior.
...
| Commerce service | TASK-025 through TASK-034, the Phase 5 `MigrationRequest` workflow, and the Phase 6 exact notification-event projection are implemented and verified locally in `Z:\GitHub\zoolanding-commerce`: policy resolution, retained storage, provider-neutral domain rules, eight literal browser routes, authorization, catalog/inventory/Checkout, integration-event inbox and notification outbox, subscription projection, reconciliation, protected migration approval/control, and isolated manual fiscal intake; no remote or AWS deployment exists yet |
| Integrations service | TASK-035 through TASK-053 plus Phase 6 SMTP connection registration, operator-only activation, and private resolution are implemented and verified locally in `Z:\GitHub\zoolanding-integrations`: connection/Stripe boundaries, provider mappings, webhook/outbox processing, subscription commands, and the bounded asynchronous bulk-migration engine; no remote, AWS deployment, or provider activation exists yet |
| Notifications service | TASK-054 through TASK-058 are implemented and verified locally in `Z:\GitHub\zoolanding-notifications`: exact-version policy resolution, closed event validation, immutable recipient lookup, private SMTP connection resolution, bounded rate/circuit state, ambiguity-safe delivery, retained 90-day technical ledger, queue/DLQ, alarms, and operator reconciliation tooling; no remote, AWS deployment, secret, or SMTP delivery exists yet |
...
- Phases 4 through 7 are local-only. Live connected accounts, webhook endpoints, secrets, stacks, routes, queues, tables, and all other AWS/provider resources remain Phase 8 deployment work; pilot configuration and activation remain Phase 9. Both require explicit authorization. The retained Commerce `MigrationRequest` history also requires an approved production retention/deletion policy, access policy, and cleanup runbook before activation.
