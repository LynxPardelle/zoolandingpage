...
For SSR, published `GET` and `HEAD` requests to `routes[].auth.required = true` routes are redirected before Angular renders the protected page shell when the route uses traditional browser-side auth metadata. The server uses `routes[].auth.redirectTo` first, then `runtime.auth.loginPath`, requires the target to be a safe same-origin path, and preserves only `draftDomain`, `debugWorkspace`, and `lang` so shared test previews keep their context without carrying ad or sensitive query params into login routes. Drafts that use the auth-admin BFF through `runtime.authRemote` or `runtime.auth.session.mode: "server-cookie"` skip this SSR redirect so Angular can call `/auth/session/me` with the HttpOnly session context before deciding whether to render or redirect. Those protected BFF routes must still render with `robots` set to `noindex,nofollow`; the SSR decorator also treats `sitemap.excludePaths` and technical robots-disallow prefixes as noindex signals.
...
`runtime.auth.session.routeAccessCacheMs` is optional public UX metadata for server-cookie drafts. Angular may reuse the sanitized `/auth/session/me` result for fast navigation between protected routes for that many milliseconds, clamped from `0` to `60000`; omit it for the app default and set `0` when a draft needs every route transition to revalidate immediately. This cache never authorizes backend reads or mutations: every protected BFF/API action must still validate the HttpOnly session, CSRF, tenant/profile headers, and current groups server-side.
