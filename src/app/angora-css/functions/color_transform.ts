...
export const color_transform = {
  ...
  HSLToRGB(HSL: string): string {
    ...
    return `rgb${rgbSplited[3] && rgbSplited[3] !== '' ? 'a' : ''}(${r},${g},${b}${
  ...
  HWBToRGB(HWB: string): string {
    ...
    return `rgb${rgbSplited[3] && rgbSplited[3] !== '' ? 'a' : ''}(${Math.round(r * 255)},${Math.round(
      g * 255
    )},${Math.round(b * 255)}${rgbSplited[3] && rgbSplited[3] !== '' ? `,${rgbSplited[3]}` : ''})`;
  },
  shadeTintColor(rgb: number[], percent: number): number[] {
    let R: any = rgb[0] === 0 && percent > 0 ? 16 : rgb[0] === 255 && percent < 0 ? 239 : rgb[0];
    let G: any = rgb[1] === 0 && percent > 0 ? 16 : rgb[1] === 255 && percent < 0 ? 239 : rgb[1];
    let B: any = rgb[2] === 0 && percent > 0 ? 16 : rgb[2] === 255 && percent < 0 ? 239 : rgb[2];
  ...
  getShadeTintColorOrGradient(tintValue: number, value: string): string {
    if (value.includes('gradient')) {
      ...
      return `rgba(${color_transform.shadeTintColor(color_transform.colorToRGB(value), tintValue).join(',')})`;
    }
  },
  separateColor4Transform(value: string): RegExpMatchArray | null {
    ...
    const colorReg: RegExp = new RegExp(/(?:(#[A-Fa-f0-9]{3,8})|(?:(rgb)|(hsl)|(hwb))a?\([0-9\.\,\s%]*\))/gi);
