...
export const doCssCreate = async (updateClasses2Create?: string[]): Promise<number> => {
  try {
    ...
    for (let class2Create of classes2Create) {
      [class2Create, bpsStringed, classes2CreateStringed] = Object.values(
        await parseClass(class2Create, bpsStringed, classes2CreateStringed, !updateClasses2Create)
      );
    }
    multiLog([
