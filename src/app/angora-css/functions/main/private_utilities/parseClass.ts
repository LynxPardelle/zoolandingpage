...
export const parseClass = async (
  ...
  let comboCreatedKey: string | undefined = Object.keys(values.combosCreated).find(cC => {
  ...
  if (!!comboCreatedKey) {
    let comboKeyCypherReg = new RegExp(values.encryptComboCreatedCharacters, 'g');
  ...
  if (class2CreateStringed.includes('{') && class2CreateStringed.includes('}')) {
    if (hasBP === true) {
      class2CreateStringed = class2CreateStringed.replace(new RegExp(values.separator, 'g'), '');
