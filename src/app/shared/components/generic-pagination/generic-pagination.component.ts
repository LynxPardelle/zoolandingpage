...
import { ChangeDetectionStrategy, Component, DestroyRef, REQUEST, computed, inject, input, signal } from '@angular/core';
...
import type { TGenericPaginationConfig, TGenericPaginationPageItem } from './generic-pagination.types';
...
export class GenericPaginationComponent {
    ...
    readonly currentPage = computed(() => this.readPageParam(this.pageParam(), this.defaultPage(), this.pageIndexBase()));
    readonly currentPageSize = computed(() => this.readPositiveIntegerParam(this.pageSizeParam(), this.defaultPageSize()));
    readonly filteredItems = computed(() => this.resolveFilteredItems());
    readonly totalItems = computed(() => this.resolveTotalItems());
    readonly pageCount = computed(() => Math.max(1, Math.ceil(this.totalItems() / this.currentPageSize())));
    ...
    constructor() {
        ...
        const refreshLocation = () => this.locationRevision.update((value) => value + 1);
    ...
    id(): string | null {
        return resolveComponentRootDomId(this.config().id, this.componentId(), 'pagination') ?? null;
    }
    ...
    linkClasses(item?: TGenericPaginationPageItem): string {
        ...
        if (item?.kind === 'page' && item.active) {
            return this.joinClasses(base, this.asString(this.config().activeLinkClasses));
        }
        return base;
    }

    disabledLinkClasses(): string {
        return this.joinClasses(this.asString(this.config().linkClasses), this.asString(this.config().disabledLinkClasses));
    }
    ...
    previousHref(): string {
        return this.hrefForPage(Math.max(this.pageIndexBase(), this.boundedPage() - 1));
    }
    ...
    private resolveFilteredItems(): readonly unknown[] {
        ...
        return resolveLoopCollectionViewItems(items, view, this.loopOptions(), { applyPagination: false });
    }

    private resolveTotalItems(): number {
        if (this.shouldUseConfiguredTotal()) {
            const configuredTotal = this.asPositiveInteger(this.variables.get(String(this.config().totalItemsPath ?? '').trim()));
    ...
    private buildPageItems(): readonly TGenericPaginationPageItem[] {
        ...
        const maxLinks = Math.max(5, this.asPositiveInteger(this.config().maxNumericLinks) ?? 7);
        const firstPage = pageIndexBase;
        const lastPage = pageCount + pageIndexBase - 1;

        if (pageCount <= maxLinks) {
            return this.range(firstPage, lastPage).map((page) => this.pageItem(page, currentPage));
        }
        ...
        const items: TGenericPaginationPageItem[] = [this.pageItem(firstPage, currentPage)];
        if (start > firstPage + 1) items.push({ kind: 'ellipsis', label: '...' });
        this.range(start, end).forEach((page) => items.push(this.pageItem(page, currentPage)));
        if (end < lastPage - 1) items.push({ kind: 'ellipsis', label: '...' });
        items.push(this.pageItem(lastPage, currentPage));
        return items;
    }

    private pageItem(page: number, currentPage: number): TGenericPaginationPageItem {
        return {
            kind: 'page',
            page,
            label: String(page),
    ...
    private hrefForPage(page: number): string {
        ...
        const targetPath = this.asString(this.config().path) || currentUrl.pathname || '/';
        const params = new URLSearchParams(currentUrl.searchParams);
        params.set(this.pageParam(), String(page));
        params.set(this.pageSizeParam(), String(this.currentPageSize()));

        const search = params.toString();
        const fragment = this.normalizedFragment(this.asString(this.config().fragment) || currentUrl.hash);
    ...
    private loopOptions(): TLoopMaterializationOptions {
        return {
            ...
            getI18n: () => undefined,
            getQueryParam: (key) => this.currentUrl().searchParams.get(key) ?? undefined,
    ...
    private defaultPage(): number {
        return this.asPositiveInteger(this.config().defaultPage) ?? this.pageIndexBase();
    }
    ...
    private readPageParam(key: string, fallback: number, pageIndexBase: 0 | 1): number {
        const raw = this.currentUrl().searchParams.get(key);
        const parsed = Number(raw ?? fallback);
        if (!Number.isFinite(parsed) || parsed < pageIndexBase) return pageIndexBase;
