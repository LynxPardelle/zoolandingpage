...
const OPAQUE_SECRET_REFERENCE_PATTERN = /^(?!.*\/\/)(?!.*(?:^|\/)\.{1,2}(?:\/|$))(?:(?:\/)?(?:[A-Za-z0-9_.+=@-]+\/)+[A-Za-z0-9_.+=@-]+|arn:(?:aws|aws-us-gov|aws-cn):(?:ssm:[a-z0-9-]+:\d{12}:parameter\/|secretsmanager:[a-z0-9-]+:\d{12}:secret:)[A-Za-z0-9_.+=@\/-]+)$/;
