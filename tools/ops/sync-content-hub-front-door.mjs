...
function hostRule(hosts) {
  ...
  if (!normalizedHosts.length) throw new Error('At least one content-hub host is required');
  return {
...
function renderRouterSet({ name, hosts, endpoint, certResolver = DEFAULT_CERT_RESOLVER }) {
  const serviceName = `zlp-content-hub-${name}`;
  const stageMiddleware = endpoint.stagePrefix ? `zlp-content-hub-${name}-stage` : null;
  const rule = routeRule(hosts);
  return [
    `    zlp-content-hub-${name}:`,
...
function renderContentHubFrontDoorConfig({
  testApiEndpoint,
  prodApiEndpoint,
  ...
  const lines = [
    '# Generated by tools/ops/sync-content-hub-front-door.mjs',
    `# Generated at ${generatedAtCentral}`,
    '# Routes only /features/content-hub/read and /features/content-hub/action.',
    'http:',
    '  routers:',
...
function buildRemoteUpdateScript({ remoteFile, yamlContent }) {
  ...
  return [
    ...
    "base64 -d > \"$tmp\" <<'ZLP_CONTENT_HUB_TRAEFIK_YAML'",
    encoded,
    'ZLP_CONTENT_HUB_TRAEFIK_YAML',
    'chmod 0644 "$tmp"',
    'mv "$tmp" "$target"',
    'echo "updated=$target"',
  ].join('\n');
...
async function sendSsmShellScript({ instanceIds, script, awsProfile, awsRegion }) {
  ...
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'zlp-content-hub-ssm-'));
  ...
  try {
    ...
    const sendResult = await runAwsJson(
      [
        ...
        'AWS-RunShellScript',
        '--comment',
        'zoolanding content-hub Traefik route sync',
        '--parameters',
        `file://${parametersPath}`,
      ],
      { profile: awsProfile, region: awsRegion },
...
async function main() {
  ...
  const testApiEndpoint = args['test-api-endpoint'] ?? env.ZOOLANDING_CONTENT_HUB_TEST_ENDPOINT ?? env.npm_config_test_api_endpoint;
  const prodApiEndpoint = args['prod-api-endpoint'] ?? env.ZOOLANDING_CONTENT_HUB_PROD_ENDPOINT ?? env.npm_config_prod_api_endpoint;
  const testHosts = splitList(args['test-hosts'] ?? env.ZOOLANDING_CONTENT_HUB_TEST_HOSTS ?? env.npm_config_test_hosts);
  const prodHosts = splitList(args['prod-hosts'] ?? env.ZOOLANDING_CONTENT_HUB_PROD_HOSTS ?? env.npm_config_prod_hosts);
  const remoteFile = args['remote-traefik-file'] ?? env.ZOOLANDING_CONTENT_HUB_TRAEFIK_FILE ?? env.npm_config_remote_traefik_file ?? DEFAULT_REMOTE_FILE;
  ...
  const yamlContent = renderContentHubFrontDoorConfig({
    testApiEndpoint,
    prodApiEndpoint,
  ...
  if (!instanceIds.length) throw new Error('No SSM managed instance found for content-hub front-door sync');
